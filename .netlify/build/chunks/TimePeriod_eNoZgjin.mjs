...
import { e as getBreakpoints } from './helpers_CRk732sB.mjs';
/* empty css                                                           */
/* empty css                         */
...
import { d as $$Text } from './BaseLayout_D0jnvpW9.mjs';
...
class ImmutableDataStore {
  ...
  static async fromModule() {
    try {
      const data = await import('./_astro_data-layer-content_ntgEmjtm.mjs');
...
function createGetCollection({
  ...
  return async function getCollection(collection, filter) {
    ...
    if (collection in contentCollectionToEntryMap) {
      ...
      const { default: imageAssetMap } = await import('./content-assets_Cgi2AttI.mjs');
      const result = [];
...
async function renderEntry(entry) {
  ...
  if (entry.deferredRender) {
    try {
      const { default: contentModules } = await import('./content-modules_ebmhNE_T.mjs');
